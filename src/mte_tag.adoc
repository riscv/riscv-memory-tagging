...
HLVX* instructions always generate unchecked loads. HLV* and HSV* instructions
`MTE_MODE` is defined based on privilege level at which they are executing and
may generate checked memory accesses.

- HS mode: MTE_MODE for HLV*/HSV* is taken from `henvcfg.MTE_MODE`.
- HU mode: MTE_MODE for HLV*/HSV* is taken from `hstatus.HUMT`.
...
The Zimte extension adds the `HUMT` field (bit 51:50) to `hstatus`. When the
`HUMT` field is set to `0b10` or `0b11`, HLV*/HSV* may generate checked
accesses in U mode.

When `HUMT` is `0b00`, HLV*/HSV* instructions do not generate checked accesses
