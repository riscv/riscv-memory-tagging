...
Some software environments, such as M-mode firmware or tightly controlled
...
corresponding tag to tag storage may be desirable for common cases such as
small stack allocations.

This extension does not define separate non-Zimop encodings for such operations.
An implementation can instead recognize common instruction sequences that
construct a tagged pointer and then store the corresponding tag, and can fuse
them internally using microarchitectural fusion, provided that the architectural
behavior remains equivalent to executing the original instruction sequence.
...
The tag generation exclusion mask CSRs are WARL. For every supported
`pointer_tag_width`, the read-back value must have at least one active mask bit
clear, so that `gentag` always has at least one permitted tag value. The reset
value of the tag generation exclusion mask CSRs is zero.
