...
If execution environment is M-mode and MPRV=1, then load instruction collect
MTAG bit from first stage page table for code page of guest instruction in
following manner
...
of qualifying instruction is deposited in `mstatus.MTAG_I`. If in M-mode and
MPRV=1, load sets `mstatus.MTAG_I` to bitwise AND of first-stage leaf page
entry MTAG bit and execute permission bit.
...
HSX* instructions walks VS-stage page table to fetch instruction then
