...
HS mode may generate checked accesses depending on effective privilege of VS or
VU (as defined in privileged specification). Memory tagging on HLV*/HSV*
instructions in U mode is available when `hstatus.HUMT` is set.
...
and may generate checked memory accesses.

- Effective privilege is VS: xMT_MODE for HLV*/HSV* is defined by `henvcfg`.
- Effective privilege is VU: xMT_MODE for HLV*/HSV* is defined by `hstatus`.
...
The Zimt extension adds the `HUMT` field (bit 9) and `VUMT_MODE` (bit 51:50) to
`hstatus`. When the `HUMT` field is set, HLV*/HSV* may generate checked
accesses in U mode. `VUMT_MODE` selects the `xMT_MODE` if effective privilege
mode is VU.

When `HUMT` is clear, HLV*/HSV* instructions do not generate checked accesses
in U mode even if first stage paging tables have MTAG=1 in page table entry.
